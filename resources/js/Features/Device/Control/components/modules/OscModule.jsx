        ...
        <div>
            ...
            <div className="grid gap-x-4 gap-y-2 justify-items-center items-end" style={{ gridTemplateColumns: 'repeat(3, 40px)' }}>
                ...
                {isMaster
                    ...
                    : <IoButton label="Hardsync" cc={CC[getCCKey('HARDSYNC')]} value={values[getCCKey('HARDSYNC')] ?? 0} send={sendCC} appendLog={appendLog} />
                }
            </div>

            <ModuleDivider/>
            <div className="grid gap-x-4 gap-y-2 justify-items-center items-end" style={{ gridTemplateColumns: 'repeat(3, 40px)' }}>
                <IoSelector label="Wave" cc={CC[getCCKey('WAVEFORM')]} options={isMaster ? OscWavesMaster : OscWaves} value={values[getCCKey('WAVEFORM')] ?? 0} send={sendCC} appendLog={appendLog} onChange={setActiveWave} />
                <IoKnob label="Vol" cc={CC[getCCKey('VOLUME')]} value={values[getCCKey('VOLUME')] ?? 64} send={sendCC} appendLog={appendLog} />
                {isMaster
                    ? <IoKnob label="Ring" cc={CC.RING_AMOUNT} value={values.RING_AMOUNT ?? 0} send={sendCC} appendLog={appendLog} />
                    : <IoKnob label="Phase" cc={CC[getCCKey('PHASE')]} type="bipolar" value={values[getCCKey('PHASE')] ?? 64} send={sendCC} appendLog={appendLog} />
                }
            </div>

            <ModuleDivider/>
            <div className="grid gap-x-4 gap-y-2 justify-items-center items-end" style={{ gridTemplateColumns: 'repeat(3, 40px)' }}>
                <IoKnob label={isNoise ? "Color" : "Oct"} cc={CC[getCCKey('OCT')]} type={isNoise ? "unipolar" : "bipolar"} value={values[getCCKey('OCT')] ?? 64} send={sendCC} appendLog={appendLog} />
                <IoKnob label={isNoise ? "Rate" : "Tune"} cc={CC[getCCKey('TUNE')]} type={isNoise ? "unipolar" : "bipolar"} value={values[getCCKey('TUNE')] ?? 64} send={sendCC} appendLog={appendLog} />
                <IoKnob label={isNoise ? "Reso" : "Fine"} cc={CC[getCCKey('FINE')]} type={isNoise ? "unipolar" : "bipolar"} value={values[getCCKey('FINE')] ?? 64} send={sendCC} appendLog={appendLog} />
            </div>
        </div>
    );
}

export default function OscModule({ id, sendCC, appendLog, values = {} }) {
    return (
        <Module id={id} title="OSC">
            <div className="flex gap-8 justify-center">
                <OscSection prefix="OSC1" label="OSC 1" values={values} sendCC={sendCC} appendLog={appendLog} />
                <OscSection prefix="OSC2" label="OSC 2" values={values} sendCC={sendCC} appendLog={appendLog} />
                <OscSection prefix="OSC3" label="OSC 3" isMaster values={values} sendCC={sendCC} appendLog={appendLog} />
            </div>
        </Module>
    );
}